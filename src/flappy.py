...
def flappy_bird():
    ...
    bird_images = [pygame.image.load('assets/sprites/bluebird-upflap.png').convert_alpha(),
    ...
    ground_image = pygame.image.load('assets/sprites/base.png').convert_alpha()
    ...
    while begin:
        ...
        screen.blit(fps_text, (SCREEN_WIDHT - 100, 10))
    ...
    while True:
        ...
        for pipe in pipe_group:
            if pipe.rect[0] + PIPE_WIDHT < 0:
                to_remove.append(pipe)

        for item in to_remove:
            pipe_group.remove(item)

        # Optimized collision detection
        for pipe in pipe_group:
            if pipe.rect[0] + PIPE_WIDHT > 0:  # Only if pipe is on screen
